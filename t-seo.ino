...
const int SPEED = 85;                         // 0..255      SPEED = 85
const float KP = 11;                          // Máximo: ?   KP = 11
const float KD = 1;                           // Máximo: ?   KD = 1

const int MAX_CORRECTION = SPEED * 1.5;       // Limita la corrección máxima, debe ser como mucho SPEED * 2
const int CENTRO_DE_CELDA = (25 / 2) - 5.5;   // -5,5 original centro = 25cm/2 - distancia del sensor lateral desde el centro del robot

const int DISTANCIA_FRONTAL_MIN = 7;          // 7cm Distancia a la pared frontal, para evitar chocarla (en cm)
const int DELAY_GIRO_FRONTAL = 250;           // Tiempo que gira cuando detecta pared al frente (en ms). La idea es que gire 90º
...
#define MAX_DISTANCE 30 // 50cm Maximum distance we want to ping for (in centimeters). Maximum sensor distance is rated at 400-500cm.
...
void recorrer(void) {
  ...
  delay(30);                     // Wait between pings. 29ms should be the shortest delay between pings.
  ...
  if (distancia_frontal > DISTANCIA_FRONTAL_MIN) {  // Si tengo libre adelante:
    correccion_lateral(distancia_derecha);

  }
  else {                                            // Si no, giro hacia la izquierda
    motorIzq.speed(-SPEED);
    motorDer.speed(SPEED);
